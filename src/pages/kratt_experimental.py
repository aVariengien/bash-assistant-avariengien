...
highlighted_data = random_subdict(learning_data, n=1)
...
def generate_update_dict(chat_history, learning_data):
    ...
    Only include useful new vocabulary, grammar tips, or sentence structures helpful for beginner Estonian learners. Include only Estonian sentences.
    ...
    Only ouput the correct json, nothing else.
    """
...
if prompt := st.chat_input():
    ...
    if "<image_generation" in full_response:
        ...
        st.image(image)
        display_message = full_response.replace(f'<image_generation prompt="{image_prompt}">', "")
    else:
