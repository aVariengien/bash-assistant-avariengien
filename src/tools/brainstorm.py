...
async def run_conversations(prompt: str, num_conversations: int, num_turns: int, temperature: float) -> List[Dict]:
    personalities = [
        ...
        "Devil's Advocate: You challenge existing ideas and propose counterpoints.",
        "Pattern Recognizer: You identify recurring themes and extrapolate them into new contexts.",
        "Lateral Thinker: You make unexpected connections between seemingly unrelated concepts.",
        "Simplifier: You distill complex ideas into their core essence, stripping away non-essentials.",
        "Amplifier: You take small ideas and expand them to their logical extremes.",
        "Reverse Engineer: You start with ideal outcomes and work backwards to find novel solutions.",
        "Constraint Embracer: You thrive on limitations, using them as catalysts for creativity.",
        "Metaphor Maker: You translate abstract concepts into tangible, relatable comparisons.",
        "Synthesizer: You blend diverse viewpoints into cohesive, multifaceted proposals."
    ]
...
def save_results(results, prompt):
    ...
    os.makedirs("../brainstorm_output/html", exist_ok=True)
    os.makedirs("../brainstorm_output/json", exist_ok=True)
    ...
    with open("../brainstorm_output/json/prompt_history.json", "a") as f:
    ...
    with open(f"../brainstorm_output/html/visualization_{timestamp}.html", "w") as f:
